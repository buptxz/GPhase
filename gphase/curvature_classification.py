...
with open('../result/result.csv', 'w', newline='') as csv_file:
    ...
    for sample_index in range(sample_number):
        if label[sample_index] == prediction[sample_index]:
            spamwriter.writerow([label[sample_index], curvature[sample_index]])
        else:
            spamwriter.writerow([label[sample_index], curvature[sample_index], "error"])
...
#     plt.savefig("D:/xiong/Desktop/figure0504/" + str(sample + 1) + '.png', format="png", dpi=200)
#     plt.close()
