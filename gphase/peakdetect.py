...
# with open('../xrd_original_data/xrd_data.csv', 'r') as f:
#             reader = csv.reader(f)
#             xrd_data = list(reader)
# xrd_data = [[float(column) for column in row] for row in xrd_data]
# feature_number = len(xrd_data)
...
if __name__ == "__main__":
    ...
    with open('/home/zheng/Desktop/PhaseBoundary/Dataset/AlCuMo/AlCuMo_composition_long300.csv', 'r') as f:
        reader = csv.reader(f)
        comp_gt_data = list(reader)
    with open('/home/zheng/Desktop/PhaseBoundary/Dataset/AlCuMo/AlCuMo_XRD_long300.csv', 'r') as f:
        reader = csv.reader(f)
        xrd_data = list(reader)
    comp_data = []
    ground_truth = []
    for row in comp_gt_data:
        comp_data.append(row[0:3])
        ground_truth.append(int(row[3]))
    comp_data = [[float(column) for column in row] for row in comp_data]
    xrd_data = [[float(column) for column in row] for row in xrd_data]   
    sample_number = len(comp_data)
    feature_number = len(xrd_data[0])

    # _max, _min = peakdetect(yy, range(feature_number), lookahead = 10, delta = 0)
    # # _max, _min = peakdetect_zero_crossing(y, x, 21)
    # # print _max
    # # print _min
    # xm = [p[0] for p in _max]
    # ym = [p[1] for p in _max]
    ...
    # plot = pylab.plot(range(feature_number), yy)
    # #pylab.xlabel(str(ground_truth[i]))
    # #plt.title('sample ' + str(i + 1) + 'category ' + str(ground_truth[i]))
    # pylab.hold(True)
    # pylab.plot(xm, ym, 'r*')
    # pylab.plot(xn, yn, 'go')    
    # pylab.show()
